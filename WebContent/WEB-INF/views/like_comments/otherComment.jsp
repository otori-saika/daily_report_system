...
<c:import url="/WEB-INF/views/layout/app.jsp">
    <c:param name="content">
        ...
        <table id="comment">
               <c:forEach var="comments" items="${otherComments}">
               <tbody style="border: 1px solid #cccccc;">
               <tr style="border:none;">
                   <td style="border:none;padding: 5px 2%;"><c:out value="${comments.employee.name}" />&nbsp;さんからコメントされました。&nbsp;&nbsp;
               ...
               <tbody/></table>
                <div id="pagination">
                (全 ${otherCommentsCount} 件) <br />
                <c:forEach var="i" begin="1" end="${((otherCommentsCount - 1) / 15) + 1}" step="1">
                    <c:choose>
                        <c:when test="${i == page}">
                            <c:out value="${i}" />&nbsp;
                        </c:when>
                        <c:otherwise>
                            <a href="<c:url value="/otherComment?id=${employee.id}&page=${i}" />"><c:out value="${i}" /></a>&nbsp;
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
            </div>
